...
volatile_acidity = st.number_input(
    "Volatile Acidity", min_value=0.0, value=0.7, step=0.01
)
...
free_sulfur_dioxide = st.number_input(
    "Free Sulfur Dioxide", min_value=0, value=11, step=1
)
total_sulfur_dioxide = st.number_input(
    "Total Sulfur Dioxide", min_value=0, value=34, step=1
)
...
if st.button("Readiness"):
    ...
    try:
        response = requests.get(f"{FASTAPI_URL}/readiness")

        if response.status_code == 200:
            st.success("Ready to work")
        else:
            st.error(response.text)
